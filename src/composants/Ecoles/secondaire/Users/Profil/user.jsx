    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (["Enseignant", "Enseignante", "Administrateur", "Administratrice", "Super Administrateur", "Super Administratrice"].includes(userData.fonction.name)) {
                    try {
                ...
                {isLoading ? (
                    ...
                    <div className="navbar-nav w-100">
                        <Link to="/secondaire/profil_user" className="nav-item nav-link active text-white">
                                <img src={ImageFind} style={{ height: '30px', width: '30px', marginRight: '10px' }} alt="Mon compte" /> Mon compte
                            </Link>
                            {user && (["Administrateur", "Administratrice", "Super Administrateur", "Super Administratrice"].includes(user.fonction.name) || ["Administrateur", "Administratrice", "Super Administrateur", "Super Administratrice"].includes(user.role)) && (
                                <Link to="/secondaire/bureau_admin" className="nav-item nav-link text-white">
                                    <img src={ImageEvent} style={{ height: '30px', width: '30px', marginRight: '10px' }} alt="Bureau Admin" /> Bureau Admin
                                </Link>
                            )} 
                                    ...
                                    <div className={`dropdown-content ${openMenus['menu1'] ? 'open' : ''}`}>
                                        {infoClasseUser.map(classe => (
                                            <Link
                                                ...
                                                key={classe.id}
                                            >
                                                <i className='bi bi-table'></i> {classe.classe.name} {classe.option.name}
                                            </Link>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="menu">
                            ...
                            <div className={`dropdown-content ${openMenus['menu4'] ? 'open' : ''}`}>
                               {user && ["Enseignant", "Enseignante", "Administrateur", "Administratrice", "Super Administrateur", "Super Administratrice"].includes(user.fonction.name) && (
                                ...
                                {(user && (["Elève"].includes(user.fonction.name) || user.role === 'Elève')) && (
                                    <>
