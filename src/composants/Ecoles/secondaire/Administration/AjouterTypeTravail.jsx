...
const AjouterTypeTravail = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('https://api.ecolapp.cd/api/typeTravail/create', formData, {
        headers: { 'Content-Type': 'application/json' },
      });

      if (response.data.status === 200) {
        setSuccessMessage("Type travail ajouté avec succès !");
        setErrors({});
          ...
          <section className="section d-flex flex-column align-items-center justify-content-center py-4">
            <div className="col-lg-6 col-md-8">
              <div className="card mb-3">
                <Link to='/secondaire/liste_type_travail' className='text-center btn btn-warning text-white'>Liste type travail</Link>
                <div className="card-body">
                  <h3 className="text-center" style={{ fontWeight: 900, color: '#1769ff' }}>Ajouter Type travail</h3>
                  <p className="text-center">Veuillez remplir les informations ci-dessous.</p>

                  <form onSubmit={handleSubmit}>
                    <div className="mb-3">
                      <label htmlFor="tranche">Type travail</label>
                      <input
                        type="text"
                        name="name"
...
export default AjouterTypeTravail;
