...
const PaiementEleve = () => {
  ...
  const fetchPaiements = useCallback(async (page = 1) => {
    try {
      const response = await axios.get(`http://localhost:8000/api/paiement/eleve/${id}?page=${page}`);
      setPaiements(response.data.paiements.data || []);
      setTotalPages(response.data.paiements.last_page); // Nombre total de pages
      setCurrentPage(page);
