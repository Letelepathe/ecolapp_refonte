...
const ListeModePaiement = () => {
  ...
  const handleDelete = async (id) => {
    ...
    setError('');
    try {
      const response = await axios.get(`https://api.ecolapp.com/api/mode_paiement/delete/${id}`);
              ...
              <div className="card mb-3">
                <div className="card-body">
                  <div className="justify-content-between align-items-center d-flex">
                    ...
                    <Link to='/maternelle/ajouter_mode_paiement' className='btn btn-primary mb-2 mt-2'>Ajouter</Link>
                  </div>
                </div>
                <div className="table-responsive">
                    <Link to='/maternelle/ajouter_mode_paiement' className="btn btn-primary mb-2 mt-2">Ajouter mode</Link>
                    {successMessage && ( 
                        <p> {successMessage} </p>
                    )}
                    {error ? (
