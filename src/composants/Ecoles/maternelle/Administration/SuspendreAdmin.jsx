...
const SuspendreAdmin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        const response = await axios.get(`https://api.ecolapp.cd/api/user/suspendreAdmin/${id_user}`);
            ...
            <form onSubmit={handleSubmit} className="code-form mt-4 p-4">
              ...
              {errorMessage && 
                <p className="text-danger mt-3">
                    {errorMessage} <br />
                <Link to='/maternelle/bureau_admin' className='btn btn-primary'>Retour</Link>
                </p>
              }
            </form>
          </div>
