...
const LoginMaternelle = () => {
  ...
  const soumettreFormulaire = async (evenement) => {
    ...
    try {
      ...
      if (reponse.data.status === 200) {
        ...
        naviguer("/maternelle/profil_user");
      } else {
  ...
  return (
    <>
      <Helmet>
        <title>Maternelle | Connexion</title>
      </Helmet>
      <LoginRefonte
        sousTitre="Connectez-vous à votre espace maternelle."
        valeurs={donneesForm}
        erreurs={erreurs}
        message={message}
        chargement={chargement}
        champs={champsConnexion}
        lienCreation="/maternelle/alert_creation_compte"
