...
const PaiementEnOrdre = () => {
  ...
  const handleDetteSubmit = () => {
    if (selectedMotif) {
      navigate(`/maternelle/paiement_avec_dette/${selectedMotif}`);
    } else {
      setError('Veuillez sélectionner un motif avant de valider.');
    }
  };
  ...
  useEffect(() => {
    const fetchPaiements = async () => {
      try {
        const response = await axios.get(`https://api.ecolapp.cd/api/paiement_en_ordre/motif/${motif_id}/ecole/${ecole_id}/direction/${direction}`);
        setPaiements(response.data.paiementsEnOrdre);
        setFilteredPaiements(response.data.paiementsEnOrdre); // Initialiser la liste filtrée
      } catch (error) {
      ...
      <div className="content">
        ...
        <section className="container mt-3 shadow bg-white py-3">
          <div className="justify-content-between align-items-center d-flex">
            ...
            <Link to="/maternelle/ajouter_paiement" className="btn btn-primary mb-3">
