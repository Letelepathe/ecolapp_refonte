...
const PaiementAvecDette = () => {
  ...
  const handleOrdreSubmit = () => {
    if (selectedMotif) {
      navigate(`/maternelle/paiement_en_ordre/${selectedMotif}`);
    } else {
  ...
  useEffect(() => {
    const fetchPaiements = async () => {
      try {
        const response = await axios.get(`https://api.ecolapp.cd/api/paiement_avec_dette/motif/${motif_id}/ecole/${ecole_id}/direction/${direction}`);
