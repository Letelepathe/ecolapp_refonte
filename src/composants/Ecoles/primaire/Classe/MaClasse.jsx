...
const MaClasse = () => {
  ...
  const handleCoursVideoSubmit = () => {
    if (selectedCoursVideo) {
      navigate(`/primaire/cours_video/${id_classe}/${id_option}/${selectedCoursVideo}`);
    } else {
                ...
                <div className="bg-white py-3 shadow mt-2 mb-2" style={{borderRadius:'20px'}}>
                  <div className="container">
                    ...
                    <div className="justify-content-center aling-items-center d-flex">
                      <Link to={`/primaire/ajouter_presence_eleve/${id_classe}/${id_option}`} className="btn btn-primary w-50" style={{marginRight : '5px'}}>Ajouter</Link>
                      <Link to={`/primaire/historique_presence_eleve/${id_classe}/${id_option}`} className="btn btn-warning text-white w-50">Historique</Link>
                    </div>
                  </div>
                </div>
              )}
                        ...
                        {travaux.length > 0 ? (
                          travaux.map((travail, index) => (
                            <tr key={travail.id}>
                              ...
                              <Link to={`/primaire/deposer_travail_by_eleve/${travail.id_cours}/${travail.id}`} className="btn btn-success w-100 mb-2 mt-2">
                                Déposer mon travail
                              </Link>
                              </td>
                              )}
                              {user && (user.fonction.name === 'Enseignant' || user.fonction.name === 'Enseignante' || user.fonction.name === 'Administrateur' || user.fonction.name === 'Super Administrateur' || user.fonction.name === 'Administratrice' || user.fonction.name === 'Super Administratrice') && (
                                <td>
                                  <Link to={`/primaire/liste_travaux_deposes/${travail.id}`} target="_blank" rel="noopener noreferrer" className="btn btn-warning text-white">
                      ...
                      <tbody>
                        {eleves.map((eleve, index) => (
                          ...
                          <td>{index + 1}</td>
                            ...
                            {user && (user.fonction.name === 'Enseignant' || user.fonction.name === 'Enseignante' || user.fonction.name === 'Administrateur' || user.fonction.name === 'Super Administrateur' || user.fonction.name === 'Administratrice' || user.fonction.name === 'Super Administratrice') && (
                              <td>
                                <Link target="_blank" rel="noopener noreferrer" to={`/primaire/consulter_resultat/${eleve.id}`} className="btn btn-primary">Résultats</Link>
                              </td>
                            )}
                          </tr>
                        ))}
